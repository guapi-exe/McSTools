...
(function (SpecialRenderers) {
    ...
    function getBlockMesh(block, nbt, atlas, cull) {
        ...
        const hasSpecialRenderer = ChestRenderers.has(blockName) ||
            SkullRenderers.has(blockName) ||
            SignRenderers.has(blockName) ||
            WallSignRenderers.has(blockName) ||
            HangingSignRenderers.has(blockName) ||
            WallHangingSignRenderers.has(blockName) ||
            ShulkerBoxRenderers.has(blockName) ||
            BedRenderers.has(blockName) ||
            BannerRenderers.has(blockName) ||
            WallBannerRenderers.has(blockName) ||
            blockName === 'minecraft:decorated_pot' ||
            blockName === 'minecraft:conduit' ||
            blockName === 'minecraft:bell';
        if (!hasSpecialRenderer && !isWater && !isLava && !block.isWaterlogged()) {
            return EMPTY_MESH;
        }
        const mesh = new Mesh();
        if (isWater) {
        ...
        const chestRenderer = ChestRenderers.get(blockName);
        ...
        if (blockName === 'minecraft:decorated_pot') {
            mesh.merge(decoratedPotRenderer(atlas));
        }
        const skullRenderer = SkullRenderers.get(blockName);
        ...
        const signRenderer = SignRenderers.get(blockName);
        ...
        const wallSignRenderer = WallSignRenderers.get(blockName);
        ...
        const hangingSignRenderer = HangingSignRenderers.get(blockName);
        ...
        const wallHangingSignRenderer = WallHangingSignRenderers.get(blockName);
        ...
        if (blockName === 'minecraft:conduit') {
            mesh.merge(conduitRenderer(atlas));
        }
        const shulkerBoxRenderer = ShulkerBoxRenderers.get(blockName);
        ...
        const bedRenderer = BedRenderers.get(blockName);
        ...
        const bannerRenderer = BannerRenderers.get(blockName);
        ...
        const wallBannerRenderer = WallBannerRenderers.get(blockName);
        ...
        if (!isWater && !isLava && block.isWaterlogged()) {
